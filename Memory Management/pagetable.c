...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if (frame == -1) { // Didn't find a free page.
		...
		if (coremap[frame].pte->frame & PG_DIRTY) {
			evict_dirty_count++;
			int write_off = swap_pageout(frame, coremap[frame].pte->swap_off);
			if (write_off == INVALID_SWAP) {
				return -1;
			}
			coremap[frame].pte->swap_off = write_off;
		} else {
			evict_clean_count++;
		}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	pgtbl_entry_t *sec_page = (pgtbl_entry_t *) (pgdir[idx].pde & PAGE_MASK);
	p = (pgtbl_entry_t *) sec_page + PGTBL_INDEX(vaddr);

	// Check if p is valid or not, on swap or not, and handle appropriately
	if (p->frame & PG_VALID) {
		hit_count++;
	} else {
		int cur_frame = allocate_frame(p);
		if (p->frame & PG_ONSWAP) {  //Frame in swap file
			swap_pagein(cur_frame, p->swap_off);
			p->frame = cur_frame << PAGE_SHIFT;
			p->frame &= (~PG_ONSWAP);
			p->frame &= (~PG_DIRTY);

		} else {  //Not in swap.
			init_frame(cur_frame, vaddr);
			p->frame = cur_frame << PAGE_SHIFT;
			p->frame |= PG_DIRTY;
		}
		miss_count++;
