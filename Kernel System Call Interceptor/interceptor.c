 ...
 *
 * Done: When a process exits, make sure to remove that pid from all lists.
...
void my_exit_group(int status) {
	spin_lock(&my_table_lock);  //Table Modified
	del_pid(current->pid);
...
asmlinkage long interceptor(struct pt_regs reg) {
	if (table[reg.ax].monitored > 0 && table[reg.ax].intercepted) {
 ...
 * 
 * DONE: Implement this function, to handle all 4 commands correctly.
 *
 * - For each of the commands, check that the arguments are valid (-EINVAL):
 *   DONE a) the syscall must be valid (not negative, not > NR_syscalls-1, and not MY_CUSTOM_SYSCALL itself)
...
int intercept(int sc);

int release(int sc);

int monitor(int sc, int pid);

int ignore(int sc, int pid);

asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	if (syscall >= 0 && syscall <= NR_syscalls - 1 && MY_CUSTOM_SYSCALL != syscall) {
		switch (cmd) {
			...
			case REQUEST_START_MONITORING:
				return monitor(syscall, pid);

			case REQUEST_STOP_MONITORING:
				return ignore(syscall, pid);
...
int intercept(int sc) {
	if (!current_uid()) {   //Root Check
		if (!table[sc].intercepted) {  //Check if intercepted already.
			spin_lock(&my_table_lock);
			table[sc].f = sys_call_table[sc]; //Save original system call
			table[sc].intercepted = 1; //Intercepted
			spin_unlock(&my_table_lock);

			spin_lock(&sys_call_table_lock);
			set_addr_rw((unsigned long) sys_call_table);
			sys_call_table[sc] = interceptor;
...
int release(int sc) {
	if (!current_uid()) {    //Root Check
		if (table[sc].intercepted) {    //Cannot de-intercept a system call that has not been intercepted yet.
			spin_lock(&my_table_lock);
			destroy_list(sc);    //table[].monitor and .listcount modified in destroy_list
			table[sc].intercepted = 0;
			spin_unlock(&my_table_lock);

			spin_lock(&sys_call_table_lock);
			set_addr_rw((unsigned long) sys_call_table);
			sys_call_table[sc] = table[sc].f;    //Restore original system call
			set_addr_ro((unsigned long) sys_call_table);
			spin_unlock(&sys_call_table_lock);
			return 0;
		} else {
...
int monitor(int sc, int pid) {
	if (pid >= 0 && table[sc].intercepted) {   //Check valid pid and call intercepted
		if (pid == 0) {
			if (!current_uid()) {
				spin_lock(&my_table_lock);
				if (table[sc].monitored != 2) {    //Check if all monitored
					table[sc].monitored = 2;
					spin_unlock(&my_table_lock);
					return 0;
				} else {
					spin_unlock(&my_table_lock);
		...
		} else if (pid_task(find_vpid(pid), PIDTYPE_PID) != NULL) {    //pid >= 1, check pid belong to valid process
			if (!current_uid() || !check_pids_same_owner(pid, current->pid)) {
				if (!check_pid_monitored(sc, pid)) {    //Check if pid monitored already
					spin_lock(&my_table_lock);
					if (!add_pid_sysc(pid, sc)) {    //Succeeded
						table[sc].monitored = 1;
						spin_unlock(&my_table_lock);
						return 0;
					} else {
						spin_unlock(&my_table_lock);
...
int ignore(int sc, int pid) {
	if (pid >= 0 && table[sc].intercepted) {   //Check valid pid and call intercepted
		if (pid == 0) {
			if (!current_uid()) {
				spin_lock(&my_table_lock);
				if (table[sc].monitored != 0) { //Check if already monitored
					destroy_list(sc);  //Stop monitoring for all
					table[sc].monitored = 0;
					spin_unlock(&my_table_lock);
					return 0;
				} else {
					spin_unlock(&my_table_lock);
			...
			if (table[sc].monitored == 2) { //Check if all pids monitored for this syscall, bonus case
				return -EINVAL;
			}
			if (!current_uid() || !check_pids_same_owner(pid, current->pid)) {
				if (check_pid_monitored(sc, pid)) {    //Check if pid monitored already
...
static int init_function(void) {
	int bookkeeping;
	spin_lock(&my_table_lock);
	for(bookkeeping = 0; bookkeeping < NR_syscalls; bookkeeping++) {
		table[bookkeeping].intercepted = 0;
		table[bookkeeping].monitored = 0;
		table[bookkeeping].listcount = 0;
		INIT_LIST_HEAD (&table[bookkeeping].my_list);
	}
	spin_unlock(&my_table_lock);
...
static void exit_function(void) {
	int sc;
	spin_lock(&my_table_lock);  //table[].monitor and .listcount modified in destroy_list
	for (sc = 0; sc < NR_syscalls; sc++) {
		destroy_list(sc);
	}
	spin_unlock(&my_table_lock);
